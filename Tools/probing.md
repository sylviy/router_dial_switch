...
本文件配合**每个交付单元自己的** `SKILL.md` 使用(一台机 / 一个任务一份,
骨架都是 `SKILL_TEMPLATE.md` 那六部分)。**人不需要读这份。**
那份 SKILL.md 的第一部分是任务表,是唯一的输入;本文件说的是怎么把它变成
能跑的东西 —— **SKILL 说干什么,这份说怎么干。**
...
Tools/            通用探针 —— 所有场景共用,零 perf 代码
    SKILL_TEMPLATE.md 空白模版:每个交付单元的 SKILL.md 都是照它的六部分长的
