...
用法(默认只切换不保存;确认回读无误后加 --apply 才真正下发):
    python Models/Mercusys_BE3600/Mercusys_BE3600.py pppoe
    python Models/Mercusys_BE3600/Mercusys_BE3600.py l2tp --apply
账号密码全部取自 config.yaml,不用敲在命令行上。

事实来源:2026-07-11 真机验证。标注 [待真机复核] 的行当时由启发式自动搞定、
没记下精确选择器 —— 失败时用 Tools 的探针取证后修正。
...
FACTS = {
    ...
    # Tools 的探针取证后,照 Tenda_AX3000.py 的 mode_overrides 补上。
}
