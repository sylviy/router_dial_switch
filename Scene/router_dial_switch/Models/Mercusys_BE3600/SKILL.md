# Mercusys BE3600 —— 这台机的任务表

**通用流程、规矩、按需询问的三种情况在 `../../SKILL.md`。**
这里只有这台机的具体值。

这张表是**从 `Mercusys_BE3600.py` 的 FACTS 抄下来的,给人读的对照**。真相以 FACTS 为准 —— 两边对不上时,信 FACTS,并把这张表改过来。
...
python tools/check_model.py Mercusys_BE3600       # 离线体检
```

这台机支持的档:dynamic / static / pppoe / l2tp / pptp。这轮测哪几档由 `config.yaml` 的 `run.dial_modes` 决定。
