...
这台机是:**LuCI(CBI):id 含点号,保存键要用表单收窄**。
...
下面每个值都是**从 `Cudy_AX3000.py` 的 FACTS 抄下来的**。真相以 FACTS 为准 ——
...
python ../../Tools/act.py         --menu "General Settings" \
    --sel "[id='cbid.network.wan.proto']" --label "DHCP(Dynamic IP)"
python ../../Tools/act.py         --menu "General Settings" \
    --sel "[id='cbid.network.wan.proto']" --label "DHCP(Dynamic IP)" \
...
python Models/Cudy_AX3000/Cudy_AX3000.py dynamic --perf     # 整轮:逐档切 + 测吞吐 + 出报告
python tools/check_model.py Cudy_AX3000       # 离线体检(过了 ≠ 验收)
```
