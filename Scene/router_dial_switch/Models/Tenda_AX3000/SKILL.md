...
这台机是:**自定义下拉(Vue / role=combobox),还有 IPv6 独立页**。
...
下面每个值都是**从 `Tenda_AX3000.py` 的 FACTS 抄下来的**。真相以 FACTS 为准 ——
...
控件:
  拨号控件:
    ...
    回读值在: [data-name='wanType']        # --value-sel,不读下拉小图标
...
```
python ../../Tools/probe_dump.py  --menu "Internet Settings"
python ../../Tools/list_modes.py  --menu "Internet Settings" --dial 'div.v-form-item:has-text("Internet Connection Type") div.v-select'
python ../../Tools/probe_count.py --menu "Internet Settings" \
    --sel 'div.v-form-item:has-text("Internet Connection Type") div.v-select' --sel "button[data-name='submit']:has(span:text-is(\"Connect\"))"
python ../../Tools/act.py         --menu "Internet Settings" --kind dropdown \
    --sel 'div.v-form-item:has-text("Internet Connection Type") div.v-select' --value-sel "[data-name='wanType']" --label "Dynamic IP"
python ../../Tools/act.py         --menu "Internet Settings" --kind dropdown \
    --sel 'div.v-form-item:has-text("Internet Connection Type") div.v-select' --value-sel "[data-name='wanType']" --label "Dynamic IP" \
    --apply-sel "button[data-name='submit']:has(span:text-is(\"Connect\"))" --reload-verify
```
...
python Models/Tenda_AX3000/Tenda_AX3000.py dynamic --perf     # 整轮:逐档切 + 测吞吐 + 出报告
python tools/check_model.py Tenda_AX3000       # 离线体检(过了 ≠ 验收)
```

这台机支持的档:dynamic / pppoe / static / dhcpv6 / pppoev6。
