...
不了。所以中间隔一个子进程:`routerctrl_bridge.py`(**py2.6 语法,别用 py3 写法去
"现代化"它**)负责下发 + 回读,只从 stdout 吐一行 JSON。本文件是那条边界的
py3 这一侧。桥接就躺在这台机自己的目录里,整个目录复制走就能跑。
...
单跑:
    ...
    python Models/TPLink_RouterCtrl/TPLink_RouterCtrl.py dynamic --apply
    python Models/TPLink_RouterCtrl/TPLink_RouterCtrl.py pptp --apply
...
自足:除了 Vendor/common/contract.py(判定)和 common/perf.py(整轮节拍 +
...
# 桥接就躺在这台机自己的目录里 —— 整个目录复制走就能跑,不依赖仓库别处。
BRIDGE = "routerctrl_bridge.py"
...
def switch(mode, cfg, hook=None):
    ...
    script = os.path.join(os.path.dirname(os.path.abspath(__file__)), BRIDGE)
