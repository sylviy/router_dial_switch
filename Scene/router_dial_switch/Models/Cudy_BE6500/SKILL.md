...
这台机是:**LuCI(CBI),但同一页有多个同名控件,选择器全部用表单收窄**。
...
下面每个值都是**从 `Cudy_BE6500.py` 的 FACTS 抄下来的**。真相以 FACTS 为准 ——
...
# ============ 第一段 · 前置 ============
前置:
  - 怎么做: 登录页密码框 "input[type=password]" 填 config.yaml 的 router.pass,然后按回车(这台机没有单独的登录按钮)
    做完的样子: 密码框消失
...
控件:
  拨号控件:
    形态: 原生下拉 <select>        # act.py 的 --kind select
    选择器: [id='cbid.network.wan.proto']
  保存键: form:has([id='cbid.network.wan.proto']) button[name='cbi.apply']
  账密框:
    pppoe_user(宽带账号): form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.username']
    pppoe_pass(宽带密码): form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.password']
    vpn_server(隧道对端地址): form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.server']
    vpn_user(隧道账号): form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.username']
    vpn_pass(隧道密码): form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.password']
...
状态表:
  ...
  - 状态名: pppoe
    ...
    刷新后回读: PPPoE
    要填什么:
      pppoe_user(宽带账号)← config.yaml 的 router.pppoe_user,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.username']
      pppoe_pass(宽带密码)← config.yaml 的 router.pppoe_pass,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.password']
  - 状态名: l2tp
    界面原话: L2TP
    刷新后回读: L2TP
    要填什么:
      vpn_server(隧道对端地址)← config.yaml 的 router.l2tp.server,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.server']
      vpn_user(隧道账号)← config.yaml 的 router.l2tp.user,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.username']
      vpn_pass(隧道密码)← config.yaml 的 router.l2tp.pass,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.password']
  - 状态名: pptp
    界面原话: PPTP
    刷新后回读: PPTP
    要填什么:
      vpn_server(隧道对端地址)← config.yaml 的 router.pptp.server,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.server']
      vpn_user(隧道账号)← config.yaml 的 router.pptp.user,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.username']
      vpn_pass(隧道密码)← config.yaml 的 router.pptp.pass,填进 form:has([id='cbid.network.wan.proto']) [id='cbid.network.wan.password']
...
```
python ../../Tools/probe_dump.py  --menu "General Settings"
python ../../Tools/list_modes.py  --menu "General Settings" --dial "[id='cbid.network.wan.proto']"
python ../../Tools/probe_count.py --menu "General Settings" \
    --sel "[id='cbid.network.wan.proto']" --sel "form:has([id='cbid.network.wan.proto']) button[name='cbi.apply']"
python ../../Tools/act.py         --menu "General Settings" \
    --sel "[id='cbid.network.wan.proto']" --label "DHCP"
python ../../Tools/act.py         --menu "General Settings" \
    --sel "[id='cbid.network.wan.proto']" --label "DHCP" \
    --apply-sel "form:has([id='cbid.network.wan.proto']) button[name='cbi.apply']" --reload-verify
```
...
python Models/Cudy_BE6500/Cudy_BE6500.py dynamic            # 只切,看回读,不下发
python Models/Cudy_BE6500/Cudy_BE6500.py dynamic --apply    # 真下发
python Models/Cudy_BE6500/Cudy_BE6500.py dynamic --perf     # 整轮:逐档切 + 测吞吐 + 出报告
python tools/check_model.py Cudy_BE6500       # 离线体检(过了 ≠ 验收)
```

这台机支持的档:dynamic / pppoe / l2tp / pptp。
