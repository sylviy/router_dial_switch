...
固件;另一台 AX3000 是 **LuCI/OpenWrt**(见 Models/Cudy_AX3000/Cudy_AX3000.py)。两份脚本
并存,别把选择器互相搬。

用法(默认只切换不保存;确认回读无误后加 --apply 才真正下发):
    python Models/Cudy_AX1500/Cudy_AX1500.py dynamic
    python Models/Cudy_AX1500/Cudy_AX1500.py pppoe
    python Models/Cudy_AX1500/Cudy_AX1500.py pppoe --apply
...
v6,只能先升级/更换固件(升级后重新用 Tools/ 的探针复核)。
