...
这台机是:**老式 frameset / 普通单文档,原生 <select>**。
...
下面每个值都是**从 `Cudy_AX1500.py` 的 FACTS 抄下来的**。真相以 FACTS 为准 ——
...
python ../../Tools/act.py         --menu "sel:#Network,sel:#WAN" \
    --sel "#wanType_id" --label "DHCP Client"
python ../../Tools/act.py         --menu "sel:#Network,sel:#WAN" \
    --sel "#wanType_id" --label "DHCP Client" \
...
python Models/Cudy_AX1500/Cudy_AX1500.py dynamic --perf     # 整轮:逐档切 + 测吞吐 + 出报告
python tools/check_model.py Cudy_AX1500       # 离线体检(过了 ≠ 验收)
```

这台机支持的档:dynamic / static / pppoe / pptp / l2tp。
