"""第 5 步:把**已经验过的东西**打印成 FACTS 骨架;--write 直接生成型号脚本。
    ...
    python tools/make_facts.py --brand Cudy --model AX1500 \\
...
每一档都该是 act.py 试切过、回读对上的。少了哪一步,骨架里就会留下
TODO,而留着 TODO 就不算过。
...
def main(argv=None):
    ...
    if todo:
        ...
        _probe.say("  每一档        -> act.py 试切过、回读对上")
        return _probe.FAIL
    ...
    if args.write:
        ...
        src = discover.model_path(SCENE, src_name)
        ...
        new_doc += ("用法(默认只切换不保存;确认回读无误后加 --apply 才真正下发):\n"
                    "    python Models/%s/%s.py %s\n"
                    "    python Models/%s/%s.py %s --apply\n"
                    "账号密码全部取自 config.yaml,不用敲在命令行上。\n\n"
                    % (stem, stem, modes[0][0] if modes else "dynamic",
                       stem, stem, modes[0][0] if modes else "dynamic"))
        new_doc += ("事实来源:Tools/ 的探针(probe_dump / list_modes /\n"
        ...
        dest = discover.model_path(SCENE, stem)
        if os.path.exists(dest):
            _probe.say("%s 已经存在,不覆盖 —— 先把旧的挪走。" % dest)
            return _probe.FAIL
        ...
        _probe.say("\n骨架里没有 TODO。--write 可以直接生成 "
                   "Models/<品牌>_<型号>/(照已交付的同原型脚本拷,"
                   "查找方式和探针一致)。")
    return _probe.PASS
