...
from common import discover, perf

MOCK_DIR = os.path.join(SCENE, "tests", "mock_router")
...
def _bridge_cases():
    ...
    tp = discover.load_model(SCENE, "TPLink_RouterCtrl")

    print("\n==== TPLink 桥接路线(假桥接,不碰真机)====")
    # 绝对路径 —— 型号脚本是拿它跟**自己的目录**拼的,给相对路径会拼歪。
    real_bridge = tp.BRIDGE
    tp.BRIDGE = os.path.join(SCENE, "tests", "mock_bridge.py")
    cfg = perf.load()
...
def main(argv=None):
    ...
    for name, model, page_file, mode, apply_it, want in CASES:
        mod = discover.load_model(SCENE, model)
