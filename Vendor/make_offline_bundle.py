...
when you bump the scene's `app/requirements.txt`, move to another Python patch
level, or need
the 32-bit build:

    python3 Vendor/make_offline_bundle.py                    # defaults below
    python3 Vendor/make_offline_bundle.py --python 3.8.10 --arch amd64
    python3 Vendor/make_offline_bundle.py --embed-zip ~/Downloads/py.zip
...
def main() -> int:
    ...
    ap.add_argument("--out", default=os.path.join(VENDOR, "python"),
                    help="where to write the runtime (default: Vendor/python)")
    ap.add_argument("--requirements", default=os.path.join(
        REPO, "Scene", "router_dial_switch", "app", "requirements.txt"))
