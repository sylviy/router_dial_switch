...
def _norm(text: Optional[str]) -> str:
    ...
    注意这里**不**折叠字符串内部的连续空白 —— 任务书写的是"规整首尾空白与
    大小写"。旧的 models/_driver._norm 还会把内部的空白折成一个空格
    (`re.sub(r"\\s+", " ", ...)`)。差别只在界面措辞内部含有连续空白/换行的
    机型上才看得出来,现有六台的 modes 措辞都是单行单空格,mock 回归不受影响。
    要恢复旧行为就把 split()/join() 那一行放回来 —— 是否恢复请人定,别默认改。
    """
