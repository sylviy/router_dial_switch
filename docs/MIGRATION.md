...
| `no_apply` | **没有了**。要不要下发由命令决定:`models/X.py <档>` 默认不下发,加 `--apply` 才下发;整轮必定下发。写在配置里的"下发开关"总有一天会有人忘了关。 |
| `brand` / `model`(profile 提示) | `run.model`(型号脚本名,即 `models/<这里>.py`) |
...
`models/TPLink_RouterCtrl.py` 的 `BRIDGE_MODE` 里。
...
| `python run_matrix.py --model X` | `python models/X.py <档> --perf`,或 `start.bat` 菜单 3 |
| `python run_matrix.py --demo` | `start.bat` 菜单 3,把 `run.backend` 设成 `simulate` |
| `python models/X.py <档> --apply` | 一样 |
| `python models/X.py <档> --param k=v` | **没有了** —— 账密从 `config.yaml` 取 |
| `python tests/smoke_test.py` | `python tests/mock_test.py`,或 `start.bat` 菜单 4 |
| `python tools/check_model.py --all` | `python skill/tools/check_model.py --all` |
| `python tools/probe_router.py --dump …` | `python skill/tools/probe_dump.py …`(拆成了探/列/验/试切四个工具) |
| `python adapt.py` | 照 `skill/SKILL.md` 那张表跑 `skill/tools/` 里的工具 |
