> **这份文件是给人看的历史记录,agent 不要加载它。**
> 适配流程看 `skill/SKILL.md`,卡住了按节读 `skill/reference.md`。
> 这里留着的是每条结论**当初是怎么得出来的**(哪天、哪台机、什么现象),
> 出争议时翻它;日常干活用不到,而且它有 39KB,读它只会挤掉真正有用的上下文。

# GOTCHAS.md — 为什么这些代码长这样

`CLAUDE.md` 是给 agent 每次会话必载的**地图**,只有指针。这个文件是那些指针
指向的地方:**理由、真机取证、硬教训**。读者是人,以及被指过来的 agent ——
**不要整个读它**,按下面的索引跳到你需要的那一节。

搬进来的内容**一字未删**(2026-08-06 从 CLAUDE.md 拆分,当时 41.9 KB,agent
每次开口前就烧掉约 1.76 万 token,而其中绝大部分读了不会改变它下一步做什么)。
段落标题保持原名,所以「见 GOTCHAS.md 的 Validated 一节」这类说法照旧成立。

## 什么时候读哪一节

| 你要做的事 | 读这一节 |
|---|---|
| 改 `models/` `matrix/` `tools/` 里的代码,想知道为什么这么切 | **Architecture** |
| 改 `_driver.py` 的查找/点击/回读逻辑 | **Gotchas** —— 每条都是真机上出过的假成功,删一条要有证据 |
| 某台机的选择器/措辞/为什么这么写 | **Validated** —— 逐台真机取证 |
| 动 `vendor/` `.bat` `chariot_perf.py`,或碰台架 | **Environment** |
| 有个功能好像没实现 / 报告里某列是空的 | **Known gaps**(别当 bug 查) |
| 想知道下一步该干什么,或某个决定的来历 | **Next steps** |
| 第一次接触这个项目,想知道它在整条测试流程里的位置 | **How it fits the test workflow**(给人的完整版在 `README.md`)|
