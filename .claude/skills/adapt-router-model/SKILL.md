...
**同一个选择器改过 3 次仍不唯一、或回读仍不对 → 停,别试第 4 次。** 清单里压根
没有这种控件也一样。这两种都是「形态」类,继续改选择器不会有结果 —— 去
`reference.md`「卡住了:先分四类」。
...
探针吃不下什么、怎么加新动词都在 `reference.md`。
**改 `_driver.py` 前先看那边的改动分级和回归要求,别直接动手。**
**验收通过后把这台的坑写回 `reference.md`** —— 唯一会累积的东西,规矩也在那节。

## 成本纪律

一台设备预算:输入 ~1500 token,输出 ~500。
**默认不读** `models/_driver.py`;**一旦判为「形态」类**,读 `reference.md`
「需要一个新动词时」那节 + `python models/_driver.py --verbs`。
**不要读** `GOTCHAS.md` 全文、`probe_*.json`、`*.png`、`reference.md` 全文
(只读要的那节)。
