...
(竞品路由器没有 HTTP API,只能这么跟 DUT 对比)。人读 `docs/README.md`。
...
| `config.yaml` | **唯一配置**。换被测机只改 `router.ip` 和 `run.dial_modes` | `docs/config.example.yaml` 的中文注释 |
...
**别整个读** `docs/GOTCHAS.md`(39KB,给人看的历史)、`vendor/`(97MB)、
`artifacts/`、`*.png`。
...
python app/start.py                        # 向导(台架上双击根目录的 start.bat)
```
...
从旧版本升上来 → `docs/MIGRATION.md`(旧的 `router.yaml` / `perf_configs/*.yaml`
哪一项去了新文件哪一节)。
